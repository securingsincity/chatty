...
Commander.prototype._init = function () {
  var self = this;
  self._loadScripts();
  self._whenInited = new RSVP.all([
    self._webhooks.update()
  ]);
};
...
Commander.prototype._loadScripts = function () {
  ...
  if (fs.existsSync(self.dir)) {
    ...
    self.logger.info(self.help({format: 'text'}));
  } else {
...
Commander.prototype._loadScript = function (path, name) {
  ...
  if (typeof module === 'function') {
    try {
      ...
      self.variables = function (vars) {
        self.currentScript.defineVariables(vars);
      };
...
Commander.prototype.help = function (options) {
  ...
  function commands() {
    var commands = _.map(self._commands, function (def) {
  ...
  function spies() {
    var spies = _.map(self._spies, function (def) {
...
Commander.prototype.updateScript = function (clientInfo, name, script) {
  var self = this;
  var tenant = Tenant.create(clientInfo, self.addon);
  return self._whenInited.then(function () {
    return self._scripts[name].update(tenant, script);
  });
};
