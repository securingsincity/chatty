...
SpyEvent.prototype.toString = function () {
  return toString(this, 'triggered spy', this.definition.hear);
};
...
CommandEvent.prototype.toString = function () {
  return toString(this, 'executed command', this.command);
};
...
function toString(event, verbNoun, subject) {
  return event.from.name +
    ' ' + verbNoun +
    ' ' + subject +
    ' from script ' + event.definition.script.name +
    ' in room ' + event.tenant.groupName + ':' + event.tenant.roomId;
}
