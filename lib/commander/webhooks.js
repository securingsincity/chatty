              ...
              trier.attempt({
                ...
                action: function (done) {
                  self.updateTenant(clientInfo, function (err) {
                    if (err) {
                      self.logger.warn('Failed to update tenant webhooks (will retry):' + clientInfo.clientKey);
                    } else {
                      complete = true;
                    }
                    done();
  ...
  function done(registeredList) {
    ...
    RSVP.all(remove.map(function (webhook) {
      ...
      if (remove.length > 0 || add.length > 0) {
        self.hipchat.sendMessage(clientInfo, clientInfo.roomId, self.addon.descriptor.name + '\'s commands and spies have been updated');
      }
      callback();
    }, function () {
