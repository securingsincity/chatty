...
Script.prototype._runLifecycle = function (op, tenant, store, send) {
  ...
  return new RSVP.Promise(function (resolve, reject) {
    ...
    function run() {
      var response = Response.create(send, self._conf.opts);
...
Script.prototype.update = function (tenant, store, send, json) {
  var self = this;
  return self.isEnabled(tenant).then(function (oldEnabled) {
    ...
    if (oldEnabled !== newEnabled) {
      tasks.push(self[newEnabled ? 'start' : 'stop'](tenant, store, send));
      send('Script ' + self.name + ' has been ' + (newEnabled ? 'enabled' : 'disabled'), {
        color: newEnabled ? 'green' : 'red'
      });
    }
