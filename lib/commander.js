...
Commander.prototype.execute = function(event, send) {
  ...
  try {
    def.action(event, new Response(send, event), new Store(this.addon.settings, event));
  } catch (e) {
...
function Store(settings, event) {
  this._settings = settings;
  this._type = event.definition.type;
  this._id = event.definition.id;
  this._tenant = event.clientInfo.clientKey;
}
...
Store.prototype.get = function (key) {
  return this._settings.get(this._key(key), this._tenant);
};

Store.prototype.set = function (key, value) {
  return this._settings.set(this._key(key), value, this._tenant);
};

Store.prototype.del = function (key) {
  return this._settings.del(this._key(key), this._tenant);
};
