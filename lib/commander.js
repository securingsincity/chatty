  ...
  return new RSVP.Promise(function (resolve, reject) {
    self.addon.settings.client.keys('*:clientInfo', function (err, keys) {
      ...
      keys.forEach(function (key) {
        self.addon.settings.client.get(key, function (err, clientInfo) {
          if (err) {
            return self.logger.error('Error retrieving clientInfo for key: ' + key + '\n' + (err.stack || err));
          }
          ...
          if (!clientInfo.roomId) {
            ...
            trier.attempt({
              ...
              action: function (done) {
                self._tryUpdateWebhooks(JSON.parse(clientInfo), function (err) {
                  if (err) {
                    self.logger.error('Error registering webhooks with tenant: ' + key + ' (will retry later):\n' + (err.stack || err));
                  } else {
                    complete = true;
                    resolve();
                  }
...
Commander.prototype.help = function (type) {
  ...
  function spies() {
    ...
    return '<b>Spies</b><pre>' + escapeHtml(cliff.stringifyRows(spies)) + '</pre>';
  }
  if (type === 'commands') {
    return commands();
  } else if (type === 'spies') {
