      ...
      RSVP.all(keys.map(function (key) {
        return new RSVP.Promise(function (resolveOne, rejectOne) {
          self.addon.settings.client.get(key, function (err, clientInfoJson) {
            ...
            self.logger.info('Updating webhooks for tenant:', clientInfo.clientKey);
            if (!clientInfo.roomId) {
              self.logger.info('Skipping tenant with unknown roomId:', clientInfo.clientKey);
...
Commander.prototype.help = function (type, filter) {
  var self = this;
  function filterLine(line) {
    return filter ? new RegExp('\\b' + filter.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&") + '\\b').test(line.join('    ')) : true;
  }
  function commands() {
    ...
    str = commands.length > 0 ? cliff.stringifyRows(commands) : 'No matches';
    return '<b>Commands</b>\n' + str;
  }
  function spies() {
    ...
    str = spies.length > 0 ? cliff.stringifyRows(spies) : 'No matches';
    return '<b>Spies</b>\n' + escapeHtml(str);
  }
  if (type === 'commands') {
